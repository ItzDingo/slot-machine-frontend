...
const elements = {
    ...
    wheelGameOverPopup: document.getElementById('wheel-game-over-popup'),
    wheelGameOverMessage: document.getElementById('wheel-game-over-message'),
    wheelGameOverResult: document.getElementById('wheel-game-over-result'),
    wheelGameOverAmount: document.getElementById('wheel-game-over-amount'),
...
function updateWheelBets() {
    ...
    document.querySelectorAll('.wheel-option').forEach(option => {
        ...
        const betInput = option.querySelector('.wheel-number-bet');
...
function lockWheelBets() {
    ...
    if (elements.wheelSpinBtn) {
        elements.wheelSpinBtn.disabled = false;
    }
}
...
async function spinWheel() {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Bet deduction failed');
        ...
        if (elements.wheelContainer) {
            elements.wheelContainer.style.transform = `rotate(${totalRotation}deg)`;
            elements.wheelContainer.style.transition = `transform ${spinDuration}ms cubic-bezier(0.17, 0.67, 0.21, 0.99)`;
        }
        ...
        setTimeout(() => {
            ...
            if (gameState.wheelGame.bets[winningSegment.value]) {
                winAmount = gameState.wheelGame.bets[winningSegment.value].amount * winningSegment.multiplier;
            }
            
            showWheelResult(winningSegment, winAmount);
...
async function showWheelResult(winningSegment, winAmount) {
    ...
    if (winAmount > 0) {
        ...
        try {
            ...
            if (!response.ok) throw new Error('Win claim failed');
...
function closeWheelGameOverPopup() {
    if (elements.wheelGameOverPopup) elements.wheelGameOverPopup.style.display = 'none';
}
...
if (document.getElementById('wheel-game-over-close')) {
    document.getElementById('wheel-game-over-close').addEventListener('click', closeWheelGameOverPopup);
}
...
// Wheel game specific event listeners
if (elements.wheelLockBtn) {
